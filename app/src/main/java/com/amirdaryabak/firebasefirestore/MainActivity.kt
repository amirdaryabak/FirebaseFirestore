package com.amirdaryabak.firebasefirestore
...
class MainActivity : AppCompatActivity() {
    ...
    private fun retrievePersons() = CoroutineScope(Dispatchers.IO).launch {
        val fromAge = if (etFrom.text.toString() != "")
            etFrom.text.toString().toInt() else 1
        val toAge = if (etTo.text.toString() != "")
            etTo.text.toString().toInt() else 99999
        try {
            ...
            withContext(Dispatchers.Main) {
                tvPersons.text = sb.toString()
                Toast.makeText(this@MainActivity, "Data is updated", Toast.LENGTH_SHORT).show()
            }
        } catch(e: Exception) {
            withContext(Dispatchers.Main) {
                Toast.makeText(this@MainActivity, e.message, Toast.LENGTH_LONG).show()
